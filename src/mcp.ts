...
import { inspectTableSchema, listTablesSchema } from './validation.js';
import { formatTableRows, handleError } from './utils.js';

export async function createMCPServer({ pool }: { pool: any }) {
  // MCP server info
  const serverInfo = {
  ...
  const mcpServer: any = new McpServer(serverInfo);
